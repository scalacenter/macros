...
trait InternalInput { self: Input =>
  ...
  private[macros] def lineToOffset(line: Int): Int = {
    // NOTE: The length-1 part is not a typo, it's to accommodate the sentinel value.
    if (!(0 <= line && line <= cachedLineIndices.length - 1)) {
      val allowedLines = s"[0..${cachedLineIndices.length - 1}]"
      val message = s"$line is not a valid line number, allowed $allowedLines"
      throw new IllegalArgumentException(message)
    }
    cachedLineIndices(line)
  }

  private[macros] def offsetToLine(offset: Int): Int = {
    val chars = this.chars
