...
private[scala] trait Dialects {
  ...
  implicit val Sbt0136: Dialect = Scala210.copy(
    ...
    toplevelSeparator = scala.meta.internal.prettyprinters.EOL
  )
  ...
  private[scala] def QuasiquoteTerm(underlying: Dialect, multiline: Boolean): Dialect = {
    require(!underlying.allowUnquotes)
    underlying.copy(allowTermUnquotes = true, allowMultilinePrograms = multiline)
  }

  // TODO: https://github.com/scalameta/scalameta/issues/380
  private[scala] def QuasiquotePat(underlying: Dialect, multiline: Boolean): Dialect = {
    require(!underlying.allowUnquotes)
